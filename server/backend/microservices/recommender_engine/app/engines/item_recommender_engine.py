# app/engines/item_recommender_engine.py

import pandas as pd
import numpy as np

import tensorflow as tf
# tf.config.run_functions_eagerly(True)  # Ensure functions run eagerly
...
@tf.function
def train_step(model: Model, optimizer, anchor_batch, positive_batch, negative_batch):
...
def train_triplet_model(model: Model,
                         anchor_features, positive_features, negative_features, epochs=100, batch_size=32):
...
@tf.function
def recommend(input_features, model: Model, place_data, k=20):
    input_dict = {
...
# def recommend(input_features, model: Model, place_data, k=20):
#     Logger._("Generating Recommendations at recommend(input_features, model, place_data, k)...")
...
#     Logger._("Input Dictionary:" + str(input_dict), 'b')
#     print("Input Dictionary:", input_dict)

#     def predict_in_main_thread(model, input_dict):
#         # print how many devices are available
#         print("Num GPUs Available: ", len(tf.config.experimental.list_physical_devices('GPU')))
#         print("Num CPUs Available: ", len(tf.config.experimental.list_physical_devices('CPU')))
#         print("Num TPUs Available: ", len(tf.config.experimental.list_physical_devices('TPU')))
#         print("Num Accelerators Available: ", len(tf.config.experimental.list_physical_devices('Accelerator')))
#         print("Num All Devices Available: ", len(tf.config.experimental.list_physical_devices()))
#         print("All Devices Available: ", tf.config.experimental.list_physical_devices())
#         print("All Devices Available: ", tf.config.experimental.list_logical_devices())

#         Logger.print_active_threads()

#         with tf.device('/cpu:0'):
#             return model.predict(input_dict)

#     try:
#         query_embedding = predict_in_main_thread(model, input_dict)
#         Logger._("Query Embedding:" + str(query_embedding), 'b')
#         print("Query Embedding:", query_embedding)
#     except Exception as e:
#         Logger._("Error during query embedding prediction: " + str(e), 'r')
#         print("Error during query embedding prediction:", e)
#         return []
...
#             'rating': place_data['rating'].values
#         })
#         Logger._("Location Embeddings:" + str(location_embeddings), 'b')
#         print("Location Embeddings:", location_embeddings)
#     except Exception as e:
#         Logger._("Error during location embeddings prediction: " + str(e), 'r')
#         print("Error during location embeddings prediction:", e)
#         return []
...
def weighted_hybrid_recommend(input_features, user_id, content_model, cf_model, place_data, user_id_to_index, cb_weight=0.5, cf_weight=0.5, k=10, food_tags=[13,14,15,16]):
    Logger._("Generating Recommendations at weighted_hybrid_recommend(input_features, user_id, content_model, cf_model, place_data, user_id_to_index, cb_weight, cf_weight, k, food_tags)...")

    # Get content-based recommendations
    cb_recommendations = recommend(input_features, content_model, place_data, k)

    Logger._("Content-Based Recommendations:" + str(cb_recommendations), 'b')
...
# Main script
def recommend_places(user_id, triplet_model, ncf_model):
    Logger._("Rungging Item Recommendation Engine at: recommend_places(user_id)...")
    ...
    Logger._(f"Triplet model: {triplet_model}")
    Logger._(f"NCF model: {ncf_model}")
    
    # # Create and train NCF model
    # ncf_model = create_ncf_model(num_users, num_places)
    # train_ncf_model(ncf_model, user_data)

    Logger._("triplet_model summary:", 'r')
    print(triplet_model.summary())
    print()

    Logger._("ncf_model summary:", 'r')
    ...
    Logger._("Input Features:" + str(input_features) + "at recommend_places(user_id)")
