...
def recommend(input_features, model, k=20):
    input_dict = {
...
def ncf_recommend(user_id, model, place_data, user_id_to_index, k=10):
    ...
    place_indices = place_data.index.values
    
    scores = model.predict([np.full((len(place_indices),), user_index), place_indices])
...
def weighted_hybrid_recommend(input_features, user_id, content_model, cf_model, place_data, user_id_to_index, cb_weight=0.5, cf_weight=0.5, k=10):
    cb_recommendations = recommend(input_features, content_model, k)
